...
function createTray() {
  ...
  const contextMenu = Menu.buildFromTemplate([
    { label: 'Show Luniq', click: () => { win?.show(); win?.focus(); } },
    { type: 'separator' },
...
async function harvestYouTubeCookies(): Promise<boolean> {
    try {
        ...
        const lines = ['# Netscape HTTP Cookie File', '# Automatically generated by Luniq', ''];
...
app.whenReady().then(async () => {
  if (process.platform === 'win32') {
    app.setAppUserModelId('com.luniq.app'); 
  }
  ...
  protocol.handle('luniq-local', async (request) => {
    try {
  ...
  session.defaultSession.webRequest.onBeforeSendHeaders(
    ...
    (details, callback) => {
      ...
      const customUa = parsedUrl?.searchParams.get('__luniq_ua');
      if (customUa) {
